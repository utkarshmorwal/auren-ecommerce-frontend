...
export default function Wishlist() {
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[calc(100vh-5rem)] flex items-center justify-center px-6 text-center">
        <div>
          <Heart size={40} className="text-ink/20 mx-auto mb-4" strokeWidth={1.25} />
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
      <ScrollReveal>
        <h1 className="font-display text-3xl sm:text-5xl">Your wishlist</h1>
        <p className="mt-3 text-ink/60">{items.length} item{items.length !== 1 ? 's' : ''} saved.</p>
      </ScrollReveal>
